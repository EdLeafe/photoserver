...
def GET_list():
    ...
    res = crs.execute("select * from frame;")
    recs = crs.fetchall()
    for rec in recs:
        fs = rec["freespace"]
...
def register_frame():
    ...
    sql = """
            select image.name from image join frame_image
                on image.pkid = frame_image.image_id
                where frame_image.frame_id = %s;
            """
...
def status(pkid):
    crs = utils.get_cursor()
    sql = """
            select name, description, interval_time, interval_units
